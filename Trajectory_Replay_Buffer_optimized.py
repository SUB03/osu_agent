...
class Trajectory:
    ...
    def add_step(self, obs, cont_action, disc_action, reward, done, value, log_prob, cursor_pos):
    ...
    def search_state_in_trajectory(self, current_state, states_array, trajectory, return_distance=False):
        ...
        try:
            ...
            print(f"states_array shape {states_array.shape}")
            print(f"current_state {current_state.shape}")
            print(f"diff {diff.shape}")
            print(f"n_states {n_states}")
            print(f"idx {min_idx}")
            print(f"len of values {len(trajectory.values)}")
            raise
...
class TrajectoryBuffer:
    ...
    def     add(self, obs, cont_action, disc_action, reward, done, value, log_prob, cursor_pos):
        self.current_trajectory.add_step(obs, cont_action, disc_action, reward, done, value, log_prob, cursor_pos)
        if done:
    ...
    def sample(self, sampled_trajectories_indices, buffer_size_sample, batch_size=None, use_best_value=False, threshold=0.01):
        ...
        sampled_log_probs = th.stack(sampled_log_probs).squeeze(1).to(self.device)
        ...
        return self._batch_or_return_all(batch_size, sampled_observations, sampled_cont_actions, sampled_disc_actions, sampled_values, sampled_log_probs, sampled_cursors, sampled_advantages, sampled_returns)
