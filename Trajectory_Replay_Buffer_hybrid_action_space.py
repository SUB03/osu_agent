...
class Trajectory:
    ...
    def add_step(self, obs, disc_action, cont_action, reward, done, value, log_prob):
        ...
        self.disc_actions[self.current_step] = np.array(disc_action)
        self.cont_actions[self.current_step] = np.array(cont_action)
    ...
    def get_data(self):
        ...
        disc_actions = np.zeros((num_steps,) +  self.disc_action_space.shape, dtype=np.float32)
        cont_actions = np.zeros((num_steps,) + self.cont_action_space.shape, dtype=np.float32)
        ...
        for i in range(num_steps):
            ...
            disc_actions[i] = self.disc_actions[i]
    ...
    def search_state_in_trajectory(self, current_state, states_array, trajectory, return_distance=False):
        ...
        diff = states_array.squeeze() - current_state
        n_states = diff.shape[0]
        distances = np.linalg.norm(diff.reshape(n_states, -1), axis=1)
        min_idx = int(np.argmin(distances, axis=0))
...
class TrajectoryBuffer:
    ...
    def __init__(self, buffer_size, observation_space, disc_action_space, cont_actions_space, device, gamma, gae_lambda):
        self.trajectories = deque(maxlen=buffer_size)
        self.buffer_size = buffer_size
        self.observation_space = observation_space
        self.disc_action_space = disc_action_space
        self.cont_action_space = cont_actions_space
        self.gamma = gamma
        self.gae_lambda = gae_lambda
        self.device = device
        self.current_trajectory = Trajectory(observation_space, disc_action_space, cont_actions_space, device, gamma, gae_lambda)
        self.cache = []

    def add(self, obs, disc_action, cont_action, reward, done, value, log_prob):
        self.current_trajectory.add_step(obs, disc_action, cont_action, reward, done, value, log_prob)
        if done:
            self.trajectories.append(self.current_trajectory)
            self.current_trajectory = Trajectory(self.observation_space, self.disc_action_space, self.cont_action_space, self.device, self.gamma, self.gae_lambda)
    ...
    def sample(self, sampled_trajectories_indices, buffer_size_sample, batch_size=None, use_best_value=False, threshold=0.01):
        ...
        #sampled_disc_actions = self.standardize_shapes(sampled_disc_actions)
        #sampled_cont_actions = self.standardize_shapes(sampled_cont_actions)
        sampled_observations = self.to_torch(np.array(sampled_observations), self.device).squeeze(1)
        sampled_disc_actions = self.to_torch(np.array(sampled_disc_actions), self.device).squeeze(1)
        sampled_cont_actions = self.to_torch(np.array(sampled_cont_actions), self.device)
        sampled_values = self.to_torch(np.array(sampled_values), self.device)
        sampled_log_probs = self.to_torch(np.array(sampled_log_probs), self.device)
        sampled_advantages = self.to_torch(np.array(sampled_advantages), self.device)
        sampled_returns = self.to_torch(np.array(sampled_returns), self.device)

        return self._batch_or_return_all(batch_size, sampled_observations, sampled_disc_actions, sampled_cont_actions, sampled_values, sampled_log_probs, sampled_advantages, sampled_returns)
